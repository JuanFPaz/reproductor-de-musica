...
1. Explicar los pasos realizados, porque usamos document.addEventListener(DOMContentLoaded).

2. El uso del objeto Audio(), que es la playlist que le pasamos como parametro.

3. Los elementos que declaramos haciendo referencia al DOM.

4. Los eventos creados para los botones de nuestro reproductor de musica.

5. Porque escogimos un range input en ves de hacer una barra de progreson con un div (Creo que por obvias razones, es mas facil manipular un input range que un div xd)

6. Explicar el evento del audio que creamos y que es el bloque if y para que sirve (!isNaN)
